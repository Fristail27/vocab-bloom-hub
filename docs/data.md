...
- **Translations are generated too** and have not been reviewed by a translator. The schema
  ...
  Arabic (`GET /api/v1/meta` lists them under `available_languages`). The published dataset (the
  ...
  coverage of each; the Chinese and Arabic rows arrive with later revisions. Translations ship in files of their own, one per language
