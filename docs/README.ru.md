...
- **Админ-панель** (английский / русский интерфейс) с тремя разделами:
  - _Managing_ — создание и редактирование английских слов, их значений, переводов, синонимов и кратких переводов;
  - _Statistics_ — сводка по содержимому словаря;
  - _Documentation_ — встроенный справочник по модели данных.
- **REST API** с документацией Swagger/OpenAPI, защищённый единственным админским логином (httpOnly JWT-cookie или Bearer-токен).
- **Импорт / экспорт словаря** в виде NDJSON-датасетов (`POST /api/en/dictionary/import`, `GET /api/en/dictionary/export`) — весь словарь можно версионировать, передавать и переносить между окружениями.
...
- Граф связей между словами помимо синонимов: антонимы, гиперонимы/гипонимы, коллокации
