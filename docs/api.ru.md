...
- **Rate limit.** Один бюджет на IP клиента на весь префикс, `PUBLIC_API_RATE_LIMIT`
  ...
  нужны квоты на клиента, поставьте экземпляр за reverse proxy. Единственное исключение —
  собственный сайт экземпляра: его серверные запросы несут `INTERNAL_API_TOKEN` в
  `X-Internal-Token` и не считаются ([`environment.md`](./environment.md)).
- **Кэшируемость.** Каждый успешный `GET` несёт `ETag`, `Last-Modified` и `Cache-Control:
public, max-age=<PUBLIC_API_CACHE_MAX_AGE>, stale-while-revalidate=<то же>`; условные запросы отвечают `304` — см.
