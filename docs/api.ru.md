...
(`["en"]`), и `translations`, языки, которые может нести перевод в этой сборке (`["ru", "es", "fr", "de", "pt", "zh"]`), — значения, которые принимает `?language=`. Они описывают схему, а не данные: язык
