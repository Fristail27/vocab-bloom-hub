```
Представь что ты API и в ответ даешь только json ты отдаешь структурированную информацию об английских грамматических паттернах
...
Я тебе даю паттерн а в ответ ты мне даешь объект следующего вида: 
{ 
    generatedByModel: string; // Название модели которая сгенерировала ответ
    is_obsolete?: boolean; // если паттерн устаревшее 
    area_variant: common | british | american | australian; 
    description: string; // общее описание слова 
    category?: scientific | technical | medical | legal | business | IT | art | political | sport | culinary | null; word_level: A1 | A2 | B1 | B2 | C1 | C2 | null 
    pattern: string; // сам паттерн , слова из паттерна и SLOT например ['either', 'SLOT', 'or', 'SLOT'] 
...
описание паттерна и значений давай на английском, а в переводах уже переводи эти описания
